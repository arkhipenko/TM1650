 ...
 *
 *	v1.0.1:
 *		2015-04-27 - Added support of program memery (PROGMEM) to store
 *      the ASCII to Segment Code table
 *
 *	v1.0.2:
 *		2015-08-08 - Added check if panel is connected during init. All
 *      calls will be disabled is panel was not connected during init.
 *
 *  v1.1.0:
 *      2015-12-20 - Code clean up. Moved to a single header file. Added Gradual
 *      brightness method
 *  v1.1.1:
 * 		2018-1-10 - Merged keycode reading and displayInt, by @marlumme.
 * 		https://github.com/arkhipenko/TM1650/issues/1
 * ===============================================*/
...
#define TM1650_DCTRL_BASE \
  0x24  // Address of the control register of the left-most digit
...
const PROGMEM byte TM1650_CDigits[128]{
  ...
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      ...
      0x00, 0x82, 0x21, 0x00, 0x00, 0x00, 0x00, 0x02, 0x39, 0x0F, 0x00, 0x00,
      0x00, 0x40, 0x80, 0x00,  // 0x20
      0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7f, 0x6f, 0x00, 0x00,
      0x00, 0x48, 0x00, 0x53,  // 0x30
      0x00, 0x77, 0x7C, 0x39, 0x5E, 0x79, 0x71, 0x6F, 0x76, 0x06, 0x1E, 0x00,
      0x38, 0x00, 0x54, 0x3F,  // 0x40
      0x73, 0x67, 0x50, 0x6D, 0x78, 0x3E, 0x00, 0x00, 0x00, 0x6E, 0x00, 0x39,
      0x00, 0x0F, 0x00, 0x08,  // 0x50
      0x63, 0x5F, 0x7C, 0x58, 0x5E, 0x7B, 0x71, 0x6F, 0x74, 0x02, 0x1E, 0x00,
      0x06, 0x00, 0x54, 0x5C,  // 0x60
      0x73, 0x67, 0x50, 0x6D, 0x78, 0x1C, 0x00, 0x00, 0x00, 0x6E, 0x00, 0x39,
      0x30, 0x0F, 0x00, 0x00  // 0x70
};
...
TM1650::TM1650(unsigned int aNumDigits) {
  iNumDigits =
      (aNumDigits > TM1650_NUM_DIGITS) ? TM1650_NUM_DIGITS : aNumDigits;
}
...
void TM1650::setBrightness(unsigned int aValue) {
  ...
  for (int i = 0; i < iNumDigits; i++) {
    ...
    iCtrl[i] =
        (iCtrl[i] & TM1650_MSK_BRIGHT) | (iBrightness << TM1650_BRIGHT_SHIFT);
 ...
 * Subsequent characters are displayed with 1 char shift each time
 * displayRunningShift() is called
 *
...
/** Display next segment (shifting to the left) of the string set by
 * displayRunning() Starts with first N positions of the string. Subsequent
 * characters are displayed with 1 char shift each time displayRunningShift is
 * called
 *
 * returns: number of iterations remaining to display the whole string
 */
...
int TM1650::displayInt(int aI) {
  ...
  for (i = 3; i >= 0; i--) {
    setPosition(i, aI ? pgm_read_byte_near(TM1650_CDigits + aI % 10 + '0')
                      : (i == 3 ? 0x3f : 0));
    aI /= 10;
